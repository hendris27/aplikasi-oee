...
<body>
    ...
    <script>
        ...
        var API_BASES = [window.location.origin, 'http://' + window.location.hostname + ':4000'].filter(function(base, idx,
            arr) {
        ...
        function normalizeOee(item, idx, sourceIdx) {
            ...
            async function downloadOeeRow(idx) {
                ...
                qtyPh.forEach(function(qItem, idx2) {
                    ...
                    if (qItem) {
                        ...
                        worksheet.mergeCells('G' + currRow + ':H' + currRow);
                        worksheet.getCell('G' + currRow).value = Number(qItem.total_qty) || (Number(qItem
                            .good ||
                ...
                opPh.forEach(function(opItem, idx2) {
                    ...
                    worksheet.getCell('D' + currRow).value = opItem ? (opItem.model_name || d.model || '-') : (d
                        .model || '-');
                    worksheet.getCell('F' + currRow).value = normalizeTime(opItem ? opItem.model_start : d
                        .start);
                    worksheet.getCell('G' + currRow).value = normalizeTime(opItem ? opItem.model_end : d
                        .stop_time);
                ...
                var tgl = dd2.length === 3 ? dd2[2] + '-' + dd2[1] + '-' + dd2[0] : (new Date()).toISOString().slice(0,
                    10);
                ...
                function _buildOeeRowPdf(d) {
                    ...
                    function ct(txt, x, yy, w, h, opts) {
                        ...
                        doc.setTextColor(opts.color ? opts.color[0] : 0, opts.color ? opts.color[1] : 0, opts.color ?
                            opts
                            .color[2] : 0);
                    ...
                    y = hdrRow(oeeCols, y);
                    y = dataRow(oeeCols, [pct(d.oee), pct(d.qly), pct(d.pfm), pct(d.avb), pct(d.efc || d.efficiency ||
                            0),
                        pct(d.acv)
                    ], y);
                    y += gap;
                    ...
                    ph.forEach(function(h, i) {
                        y = dataRow(timeCols, [i + 1 + '.', normTime(h.model_fulltime), normTime(h
                                .model_runtime),
                    ...
                    dtLogs.forEach(function(lg, i) {
                        y = dataRow(dtCols, [
                            i + 1 + '.', lg ? normTime(lg.start) : '-', lg ? normTime(lg.stop) : '-',
                            lg ? (
                    ...
                    qPh.forEach(function(q, i) {
                        ...
                        y = dataRow(qtyCols, [i + 1 + '.', q ? Number(q.target || 0) : Number(d.target || 0),
                            q ?
                    ...
                    opPh.forEach(function(op, i) {
                        ...
                        y = dataRow(opCols, [i + 1 + '.', '-', '-', model, normTime(op ? op.model_start : d
                                .start),
            ...
            function editOee(idx) {
                ...
                var machineOpts = machineList.map(function(m) {
                    return '<option value="' + m + '"' + (d.machine === m ? ' selected' : '') + '>' + m +
                        '</option>';
                }).join('');
                ...
                var custOpts = custList.map(function(c) {
                    return '<option value="' + c + '"' + (d.customer === c ? ' selected' : '') + '>' + c +
                        '</option>';
                }).join('');
                ...
                var groupOpts = ['A', 'B', 'C'].map(function(g) {
                    return '<option value="' + g + '"' + (curGroup.toUpperCase() === g ? ' selected' : '') +
                        '>Group ' +
                        g + '</option>';
                }).join('');
            ...
            function editDowntime(idx) {
                ...
                var reasons = [
                    ...
                    "ROMWRITE / TAISI PROB", "SCREW PROBLEM", "TOP UP", "TRAINING", "TRAINING MP FCT",
                    "TRAINING MP INSERT",
                ...
                var reasonOpts = reasons.map(function(r) {
                    return '<option value="' + r + '"' + (curDetail === r ? ' selected' : '') + '>' + r +
                        '</option>';
                }).join('');
            ...
            function _buildWorkbook(tab) {
                ...
                if (tab === 'oee') {
                    ...
                    headers = ['No', 'Line', 'Machine', 'Model', 'Type', 'Detail', 'Time', 'Period', 'Tech', 'Job',
                        'Executor',
                        'Solution'
                    ];
