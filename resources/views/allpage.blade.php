        ...
        <div class="table-wrapper">
            <div class="table-scroll">
                <table id="table-oee" class="data-table">
                    ...
                    <tbody id="tbody-oee">
                        <tr><td colspan="22" class="no-data">Loading data...</td></tr>
                    </tbody>
                </table>
                ...
                <table id="table-downtime" class="data-table" style="display:none;">
                    ...
                    <tbody id="tbody-downtime">
                        <tr><td colspan="13" class="no-data">Loading data...</td></tr>
                    </tbody>
                </table>
            </div>
...
async function loadData() {
    try {
        ...
        if (r1.ok) {
            ...
            oeeData = d1.map(function(item, idx) { return normalizeOee(item, idx); });
        }
    } catch(e) { console.warn('Failed to fetch OEE from server:', e.message); }

    try {
        var r2 = await fetch(API_BASE + '/api/read-downtime');
        if (r2.ok) {
            var d2 = await r2.json();
            downtimeData = d2.map(function(item, idx) { return normalizeDowntime(item, idx); });
        }
    } catch(e) { console.warn('Failed to fetch Downtime from server:', e.message); }
...
function extractShift(op) { var m = (op||'').match(/(\d)/); return m ? m[1] : ''; }
function extractGroup(op) { var m = (op||'').match(/([ABC])/i); return m ? m[1].toUpperCase() : ''; }
...
function renderOeeTable() {
    ...
    page.forEach(function(d, i) {
        ...
        html += '<button class="btn-action btn-download" onclick="downloadOeeRow(' + globalIdx + ')" title="Download">⬇</button>';
        html += '<button class="btn-action btn-edit" onclick="editOee(' + globalIdx + ')" title="Edit">✎</button>';
        html += '<button class="btn-action btn-delete" onclick="deleteOee(' + globalIdx + ')" title="Delete">✖</button>';
        html += '</td>';
        html += '</tr>';
    });
...
function renderDowntimeTable() {
    ...
    page.forEach(function(d, i) {
        ...
        html += '<td class="action-cell">';
        html += '<button class="btn-action btn-edit"   onclick="editDowntime(' + globalIdx + ')" title="Edit">✎</button>';
        html += '<button class="btn-action btn-delete" onclick="deleteDowntime(' + globalIdx + ')" title="Delete">✖</button>';
        html += '</td>';
        html += '</tr>';
    });
...
async function downloadOeeRow(idx) {
    ...
    var grayFill    = { type:'pattern', pattern:'solid', fgColor:{argb:'FFD3D3D3'} };
    var alignCenter = { horizontal:'center', vertical:'middle', wrapText:true };
    var alignLeft   = { horizontal:'left',   vertical:'middle' };
    var fontTitle   = { name:'Arial', size:14, bold:true, underline:true };
    var fontGroup   = { name:'Arial', size:10, bold:true };
    var fontPart    = { name:'Arial', size:9,  bold:true };
    var fontIsi     = { name:'Arial', size:9,  bold:false };
    var fmtTitikDua = '@*":"';
    ...
    var formatTimeFromMs = function(ms) {
        ...
        var h = Math.floor(tSec / 3600); var m = Math.floor((tSec%3600)/60); var s = tSec%60;
    ...
    worksheet.columns = [
        {width:3.8},{width:13.8},{width:13.8},{width:13.8},
        {width:13.8},{width:13.8},{width:13.8},{width:13.8}
    ];
    ...
    titleCell.value = 'OEE DIGITAL REPORT'; titleCell.font = fontTitle; titleCell.alignment = alignCenter;

    currRow += 1; worksheet.getRow(currRow).values = [];

    currRow += 1;
    var cb = worksheet.getCell('B'+currRow);
    cb.value = 'DATE'; cb.font = fontGroup; cb.numFormat = fmtTitikDua; cb.alignment = alignLeft;
    ...
    cf.value = 'LINE'; cf.font = fontGroup; cf.numFormat = fmtTitikDua; cf.alignment = alignLeft;
    ...
    cc.value = 'CUSTOMER'; cc.font = fontGroup; cc.numFormat = fmtTitikDua; cc.alignment = alignLeft;
    worksheet.mergeCells('C'+currRow+':D'+currRow);
    worksheet.getCell('C'+currRow).value     = d.customer || '-';
    worksheet.getCell('C'+currRow).alignment = alignCenter;
    worksheet.getCell('C'+currRow).border    = { bottom:{style:'thin'} };
    var cs = worksheet.getCell('F'+currRow);
    cs.value = 'SHIFT/GROUP'; cs.font = fontGroup; cs.numFormat = fmtTitikDua; cs.alignment = alignLeft;
    worksheet.mergeCells('G'+currRow+':H'+currRow);
    ...
    currRow += 1;
    worksheet.mergeCells('A'+currRow+':B'+currRow); worksheet.getCell('A'+currRow).value = pct(d.oee);
    ...
    worksheet.mergeCells('G'+currRow+':H'+currRow); worksheet.getCell('G'+currRow).value = pct(d.acv);
    for (var col = 1; col <= 8; col++) {
        var cell = worksheet.getRow(currRow).getCell(col);
        cell.font = {name:'Arial',size:9}; cell.alignment = alignCenter; cell.border = borderThin;
    }
    currRow += 1;
    ...
    ['A'+currRow,'B'+currRow,'D'+currRow,'F'+currRow].forEach(function(pos) {
        var c = worksheet.getCell(pos); c.font = fontPart; c.border = borderThin; c.alignment = alignCenter;
    });

    var ph = (d.production_history && d.production_history.length > 0)
        ? d.production_history
        : [{ model_fulltime: normalizeTime(d.start), model_runtime: d.run_time || '-', model_downtime: d.down_time || '-' }];

    ph.forEach(function(hItem, idx2) {
        currRow += 1;
        worksheet.getCell('A'+currRow).value = (idx2+1)+'.';
        worksheet.mergeCells('B'+currRow+':C'+currRow); worksheet.getCell('B'+currRow).value = normalizeTime(hItem.model_fulltime);
        worksheet.mergeCells('D'+currRow+':E'+currRow); worksheet.getCell('D'+currRow).value = normalizeTime(hItem.model_runtime);
        worksheet.mergeCells('F'+currRow+':H'+currRow); worksheet.getCell('F'+currRow).value = normalizeTime(hItem.model_downtime);
        for (var col = 1; col <= 8; col++) {
            var cell = worksheet.getRow(currRow).getCell(col);
    ...
    sDt.fill = grayFill; sDt.font = fontGroup; sDt.alignment = alignCenter; sDt.border = borderThin;
    ...
    qtyHdrs.forEach(function(h, i) {
        ...
        c.value = h; c.font = fontPart; c.border = borderThin; c.alignment = alignCenter;
    });
    worksheet.mergeCells('G'+currRow+':H'+currRow);
    ...
    qtyPh.forEach(function(qItem, idx2) {
        ...
        if (qItem) {
            ...
            worksheet.mergeCells('G'+currRow+':H'+currRow);
            worksheet.getCell('G'+currRow).value = Number(qItem.total_qty) || (Number(qItem.good||0)+Number(qItem.nogood||0)) || 0;
        } else {
    ...
    currRow += 1;
    worksheet.getCell('A'+currRow).value = 'NO.';
    worksheet.mergeCells('B'+currRow+':C'+currRow); worksheet.getCell('B'+currRow).value = 'NG ITEM';
    worksheet.getCell('D'+currRow).value = 'LOC';
    worksheet.getCell('E'+currRow).value = 'QTY';
    worksheet.mergeCells('F'+currRow+':H'+currRow); worksheet.getCell('F'+currRow).value = 'REMARKS';
    ...
    ngLogs.forEach(function(l) {
        var key = l.reason || '-';
        if (!groupNG[key]) groupNG[key] = { reason: key, qty: 0 };
        groupNG[key].qty++;
    });
    ...
    worksheet.mergeCells('D'+currRow+':E'+currRow); worksheet.getCell('D'+currRow).value = 'MODEL';
    worksheet.getCell('F'+currRow).value = 'START';
    worksheet.getCell('G'+currRow).value = 'END';
    worksheet.getCell('H'+currRow).value = 'OUTPUT';
    ['A'+currRow,'B'+currRow,'C'+currRow,'D'+currRow,'F'+currRow,'G'+currRow,'H'+currRow].forEach(function(pos) {
        var c = worksheet.getCell(pos); c.font = fontPart; c.border = borderThin; c.alignment = alignCenter;
    });
    ...
    opPh.forEach(function(opItem, idx2) {
        ...
        worksheet.getCell('D'+currRow).value = opItem ? (opItem.model_name || d.model || '-') : (d.model || '-');
        ...
        worksheet.getCell('H'+currRow).value = opItem ? (Number(opItem.good) || 0) : (Number(d.good) || 0);
        for (var col = 1; col <= 8; col++) {
            var cell = worksheet.getRow(currRow).getCell(col);
            cell.border = borderThin; cell.font = fontIsi; cell.alignment = alignCenter;
        }
    ...
    [
        ...
        c1.value = s.titles[0]; c1.font = fontGroup; c1.alignment = alignCenter; c1.border = borderThin;
        var c2 = worksheet.getCell(s.col+(currRow+4));
        c2.value = s.titles[1]; c2.font = fontGroup; c2.alignment = alignCenter; c2.border = borderThin;
    ...
    function _buildOeeRowPdf(d) {
        ...
        var pct = function(v) {
            if (v===null||v===undefined||v==='') return '0%';
            var s=String(v).trim(); return s.includes('%')?s:(parseFloat(s)||0).toFixed(1)+'%';
        };
        ...
        var W=595, ml=28, cw=W-ml-28;
        var c = cw/8; 
        var y=28, lh=15, gap=5;
        var gray=[211,211,211], white=[255,255,255];

        function rect(x,yy,w,h,fill) {
            if(fill){doc.setFillColor(fill[0],fill[1],fill[2]);doc.rect(x,yy,w,h,'FD');}
            else{doc.setFillColor(255,255,255);doc.rect(x,yy,w,h,'FD');}
            doc.setDrawColor(160,160,160); doc.setLineWidth(0.3);
        }
        function noFillRect(x,yy,w,h) {
            doc.setDrawColor(160,160,160); doc.setLineWidth(0.3); doc.rect(x,yy,w,h,'S');
        }
        function ct(txt,x,yy,w,h,opts) {
            ...
            doc.text(lines[0], x+w/2, yy+h/2, {align:'center', baseline:'middle'});
        }
        ...
        function hdrRow(cols, yy) {
            ...
            cols.forEach(function(cl){
                ...
                ct(cl.l,x,yy,cl.w,lh,{bold:true,size:7});
                x+=cl.w;
            });
            return yy+lh;
        }
        ...
        doc.text('OEE DIGITAL REPORT', ml+cw/2, y+lh/2, {align:'center',baseline:'middle'});
        y+=lh+4;

        y+=6;

        lt('DATE :',  ml+c+2, y+lh/2, {bold:true});
        doc.line(ml+c*2, y+lh, ml+c*4, y+lh);
        ct(d.date||'-', ml+c*2, y, c*2, lh, {size:8});

        lt('LINE :', ml+c*5+2, y+lh/2, {bold:true});
        doc.line(ml+c*6, y+lh, ml+c*8, y+lh);
        ct(d.line||'-', ml+c*6, y, c*2, lh, {size:8});

        y += lh + 2;

        lt('CUSTOMER :', ml+c+2, y+lh/2, {bold:true});
        doc.line(ml+c*2, y+lh, ml+c*4, y+lh);
        ct(d.customer||'-', ml+c*2, y, c*2, lh, {size:8});

        lt('SHIFT/GROUP :', ml+c*5+2, y+lh/2, {bold:true});
        doc.line(ml+c*6, y+lh, ml+c*8, y+lh);
        ct((d.shift||'-')+'/'+(d.group||'-'), ml+c*6, y, c*2, lh, {size:8});

        y += lh + gap + 2;

        y = sectionHdr('OEE', y);
        var oeeCols=[{l:'OEE',w:c*2},{l:'QUALITY',w:c},{l:'PERFORMANCE',w:c},{l:'AVAILABILITY',w:c},{l:'EFFICIENCY',w:c},{l:'ACHIEVEMENT',w:c*2}];
        y = hdrRow(oeeCols, y);
        y = dataRow(oeeCols, [pct(d.oee),pct(d.qly),pct(d.pfm),pct(d.avb),pct(d.efc || d.efficiency || 0),pct(d.acv)], y);
        y+=gap;

        y = sectionHdr('TIME', y);
        var timeCols=[{l:'NO.',w:c},{l:'FULL TIME',w:c*2},{l:'RUNTIME',w:c*2},{l:'DOWNTIME',w:c*3}];
        y = hdrRow(timeCols, y);
        var ph=(d.production_history&&d.production_history.length>0)?d.production_history:[{model_fulltime:normTime(d.start),model_runtime:d.run_time||'-',model_downtime:d.down_time||'-'}];
        ph.forEach(function(h,i){
            y = dataRow(timeCols, [i+1+'.',normTime(h.model_fulltime),normTime(h.model_runtime),normTime(h.model_downtime)], y);
        });
        y+=gap;

        y = sectionHdr('DOWNTIME INFORMATION SHEET', y);
        var dtCols=[{l:'NO.',w:c},{l:'START',w:c},{l:'STOP',w:c},{l:'TOTAL',w:c},{l:'REMARKS / REASON',w:c*4}];
        y = hdrRow(dtCols, y);
        ...
        dtLogs.forEach(function(lg,i){
            y = dataRow(dtCols, [
                i+1+'.', lg ? normTime(lg.start) : '-', lg ? normTime(lg.stop) : '-', lg ? (lg.total || '00:00:00') : '00:00:00', lg ? (lg.detail || '-') : '-'], y);
        });
        y+=gap;

        y = sectionHdr('QTY', y);
        var qtyCols=[{l:'NO.',w:c},{l:'TARGET',w:c},{l:'UPH',w:c},{l:'IDEAL',w:c},{l:'GOOD',w:c},{l:'NOGOOD',w:c},{l:'TOTAL QTY',w:c*2}];
        y = hdrRow(qtyCols, y);
        var qPh=(d.production_history&&d.production_history.length>0)?d.production_history:[null];
        qPh.forEach(function(q,i){
            var good=q?Number(q.good||0):Number(d.good||0), ng=q?Number(q.nogood||0):Number(d.ng||0);
            y = dataRow(qtyCols,[i+1+'.',q?Number(q.target||0):Number(d.target||0),q?Number(q.uph||0):Number(d.uph||0),q?Number(q.ideal||0):Number(d.ideal||0),good,ng,good+ng],y);
        });
        y+=gap;

        y = sectionHdr('QUALITY INFORMATION SHEET', y);
        var qiCols=[{l:'NO.',w:c},{l:'NG ITEM',w:c*2},{l:'LOC',w:c},{l:'QTY',w:c},{l:'REMARKS',w:c*3}];
        y = hdrRow(qiCols, y);
        var ngLogs=d.ng_logs||[]; var groupNG={};
        ngLogs.forEach(function(l){var k=l.reason||'-';if(!groupNG[k])groupNG[k]={reason:k,qty:0};groupNG[k].qty++;});
        var ngRows=Object.values(groupNG).length>0?Object.values(groupNG):[null];
        ...
        rect(ml,     y, c*3, lh, gray); ct('OPERATOR LIST', ml,     y, c*3, lh, {bold:true,size:8});
        rect(ml+c*3, y, c*5, lh, gray); ct('OUTPUT MODEL',  ml+c*3, y, c*5, lh, {bold:true,size:8});
        y+=lh;
        var opCols=[{l:'NO.',w:c},{l:'PROCESS',w:c},{l:'NAME',w:c},{l:'MODEL',w:c*2},{l:'START',w:c},{l:'END',w:c},{l:'OUTPUT',w:c}];
        y = hdrRow(opCols, y);
        var opPh=(d.production_history&&d.production_history.length>0)?d.production_history:[null];
        opPh.forEach(function(op,i){
            var model=op?(op.model_name||d.model||'-'):(d.model||'-');
            var out=op?Number(op.good||0):Number(d.good||0);
            y = dataRow(opCols,[i+1+'.','-','-',model,normTime(op?op.model_start:d.start),normTime(op?op.model_end:d.stop_time),out],y);
        });
        y+=gap+4;
    ...
    overlay.innerHTML =
        ...
        +   '<span style="font-size:15px;font-weight:700;color:#1F3864;">Preview — OEE Report: ' + (d.model||'-') + ' (' + (d.date||'') + ')</span>'
        +   '<button id="row-close-x" style="background:none;border:none;font-size:24px;cursor:pointer;color:#555;">✕</button>'
        + '</div>'
        + '<iframe src="' + pdfUrl + '" style="flex:1;width:100%;border:1px solid #ccc;border-radius:6px;min-height:0;"></iframe>'
    ...
    function closeRow() { URL.revokeObjectURL(pdfUrl); document.body.removeChild(overlay); }
    document.getElementById('row-close-x').onclick   = closeRow;
    document.getElementById('row-close-btn').onclick = closeRow;
    overlay.addEventListener('click', function(e) { if (e.target === overlay) closeRow(); });
...
function editOee(idx) {
    ...
    var modelSet = {};
    try { modelSet = JSON.parse(localStorage.getItem('type_presets') || '{}'); } catch(e) {}
    var modelList = Object.keys(modelSet);
    unique(oeeData, 'model').forEach(function(m) { if (modelList.indexOf(m) === -1) modelList.push(m); });
    modelList.sort();
    ...
    var custSet = {};
    Object.values(modelSet).forEach(function(p) { if (p.customer) custSet[p.customer] = 1; });
    unique(oeeData, 'customer').forEach(function(c) { custSet[c] = 1; });
    ...
    var shiftOpts = ['1','2','3'].map(function(s) {
        return '<option value="' + s + '"' + (curShift === s ? ' selected' : '') + '>Shift ' + s + '</option>';
    }).join('');
    var groupOpts = ['A','B','C'].map(function(g) {
        return '<option value="' + g + '"' + (curGroup.toUpperCase() === g ? ' selected' : '') + '>Group ' + g + '</option>';
    }).join('');

    Swal.fire({
        title: 'Edit OEE Data', width: '680px',
        allowOutsideClick: false, showCancelButton: true, confirmButtonText: 'Save',
            ...
            document.getElementById('eo-model').addEventListener('change', function() {
                ...
                if (preset && preset.customer) {
                    ...
                    for (var i = 0; i < custSel.options.length; i++) {
                        if (custSel.options[i].value === preset.customer) { custSel.selectedIndex = i; break; }
                    }
                }
            });
        },
        ...
        var origIdx = oeeData.findIndex(function(o) { return o.id === d.id; });
        if (origIdx !== -1) {
...
function deleteOee(idx) {
    ...
    if (!d) return;
    Swal.fire({
        title: 'Delete this data?', icon: 'warning',
        showCancelButton: true, confirmButtonText: 'Delete', confirmButtonColor: '#E8083E'
    }).then(async function(result) {
        if (!result.isConfirmed) return;
        await fetch(API_BASE + '/api/delete-oee?id=' + d.id, { method: 'DELETE' });
        loadData();
    });
}
...
function editDowntime(idx) {
    ...
    var reasonCategoryMap = {
        "5S":"LOST","CHANGE LABEL / RIBBON":"LOST","MEETING":"LOST","MP FCV OJT":"LOST","MP FV OJT":"LOST",
        "MP INSERT OJT":"LOST","ODEN CHECK":"LOST","PREPARE LINE":"LOST","TOP UP":"LOST","TRAINING":"LOST",
        "TRAINING MP FCT":"LOST","TRAINING MP INSERT":"LOST","ELECTRICAL STAGE PROBLEM":"DOWN",
        "EQUIPMENT / M/C PROBLEM":"DOWN","EQUIP SOLDER PROBLEM":"DOWN","FCT / ICT / LIGHT PROBLEM":"DOWN",
        "JIG / PALLET PROBLEM":"DOWN","KEYENCE PROBLEM":"DOWN","MATERIAL PROBLEM":"DOWN","NG FCT":"DOWN",
        "NG PALLET":"DOWN","OVER CHANGE MODEL":"DOWN","PROBLEM ALARM SELBO":"DOWN","PROBLEM CGS":"DOWN",
        "PROBLEM NG TRAY":"DOWN","PROBLEM SOLDERABILITY":"DOWN","PROBLEM W/T SELBO / SELECTIVE":"DOWN",
        "QUALITY PROBLEM":"DOWN","ROMWRITE / TAISI PROB":"DOWN","SCREW PROBLEM":"DOWN",
        "WAITING COATING/CURING":"DOWN","WAITING ENGINEERING":"DOWN","WAITING FCT COMMON":"DOWN",
        "WAITING MATERIAL":"DOWN","WAITING PACKAGING":"DOWN","WAITING PALLET":"DOWN",
        "WAITING PCB":"DOWN","WAITING TEMPERATURE":"DOWN","WAITING TRAY":"DOWN"
    };
    ...
    Swal.fire({
        title: 'Edit Downtime Data', width: '680px',
        allowOutsideClick: false, showCancelButton: true, confirmButtonText: 'Save',
        html: `<div style="text-align:left;font-size:13px;">
            <div style="display:flex;gap:10px;margin-bottom:4px;">
                <div style="flex:1;">
                ...
                if (val === 'OTHERS (CUSTOM INPUT)') {
                    ...
                    if (auto) {
                        ...
                        for (var i = 0; i < typeSel.options.length; i++) {
                            if (typeSel.options[i].value === auto) { typeSel.selectedIndex = i; break; }
                        }
                    }
                }
            });
        ...
        preConfirm: function() {
            ...
            if (detailVal === 'OTHERS (CUSTOM INPUT)') {
                ...
                if (!custom) { Swal.showValidationMessage('Please enter a custom reason'); return false; }
        ...
        var origIdx = downtimeData.findIndex(function(o) { return o.id === d.id; });
        if (origIdx !== -1) {
...
function deleteDowntime(idx) {
    ...
    Swal.fire({
        ...
        if (!result.isConfirmed) return;
        await fetch(API_BASE + '/api/delete-downtime?id=' + d.id, { method: 'DELETE' });
        loadData();
    });
}
...
function renderPagination(total) {
    ...
    document.getElementById('pag-info').textContent = 'Showing ' + (total ? start + '–' + end : '0') + ' of ' + total + ' records';
...
function goPage(n) { currentPage = n; currentTab === 'oee' ? renderOeeTable() : renderDowntimeTable(); }
function changePageSize(val) { pageSize = parseInt(val); currentPage = 1; applyFilters(); }
...
function _buildWorkbook(tab) {
    var workbook = new ExcelJS.Workbook();
    var borderThin = { top:{style:'thin'},left:{style:'thin'},bottom:{style:'thin'},right:{style:'thin'} };
    var headerFill = { type:'pattern', pattern:'solid', fgColor:{argb:'FF1F3864'} };
    var fillEven   = { type:'pattern', pattern:'solid', fgColor:{argb:'FFFCE4D6'} };
    var fillOdd    = { type:'pattern', pattern:'solid', fgColor:{argb:'FFFFFFFF'} };
    var fontHeader = { name:'Arial', size:9, bold:true, color:{argb:'FFFFFFFF'} };
    var fontBody   = { name:'Arial', size:9 };
    ...
    function addSheet(wb, sheetName, headers, colWidths, dataRows) {
        ...
        hRow.height = 30;
        hRow.eachCell(function(cell) { cell.fill=headerFill; cell.font=fontHeader; cell.alignment=alignC; cell.border=borderThin; });
        dataRows.forEach(function(rowData, idx) {
            var no = idx + 1;
            var row = ws.addRow([no].concat(rowData));
            row.height = 20;
            row.eachCell(function(cell) { cell.fill=no%2===0?fillEven:fillOdd; cell.font=fontBody; cell.alignment=alignC; cell.border=borderThin; });
        });
        ws.columns.forEach(function(col, i) { col.width = colWidths[i] || 12; });
        ws.views = [{ state:'frozen', ySplit:1 }];
    }

    var headers, widths, rows, fileName, sheetName;
    if (tab === 'oee') {
        headers   = ['No','Line','Machine','Operator','Model','Customer','Start','OEE','Availability','Performance','Quality','Achievement','Real Cycle','Std Cycle','Good Qty','NG Qty','Run Time','Down Time','Setup Time','Stop Time'];
        ...
        headers   = ['No','Line','Machine','Model','Type','Detail','Time','Period','Tech','Job','Executor','Solution'];
    ...
    return { workbook, headers, rows, fileName };
}
...
function _makePdfDoc(tab, built) {
    ...
    doc.autoTable({
        startY: 35,
        head: [built.headers],
        body: built.rows.map(function(r, i) { return [i+1].concat(r); }),
        styles: { fontSize: 7, cellPadding: 3, halign:'center', valign:'middle', overflow:'linebreak' },
...
function exportExcel() {
    ...
    overlay.innerHTML =
        '<div style="background:#fff;border-radius:10px;width:96vw;height:92vh;max-width:1300px;padding:16px 20px 14px;box-shadow:0 8px 40px rgba(0,0,0,0.4);display:flex;flex-direction:column;gap:10px;">'
        + '<div style="display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">'
        +   '<span style="font-size:15px;font-weight:700;color:#1F3864;">Preview — ' + label + ' (' + built.rows.length + ' records)</span>'
        +   '<button id="exp-close-btn" style="background:none;border:none;font-size:24px;cursor:pointer;color:#555;line-height:1;">✕</button>'
        + '</div>'
        + '<iframe id="exp-iframe" src="' + pdfUrl + '" style="flex:1;width:100%;border:1px solid #ccc;border-radius:6px;min-height:0;"></iframe>'
        + '<div style="display:flex;gap:10px;justify-content:flex-end;flex-shrink:0;">'
